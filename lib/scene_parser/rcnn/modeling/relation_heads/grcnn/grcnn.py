...
class GRCNN(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        if self.update_step > 0:
            self.gcn_collect = _GraphConvolutionLayer_Collect(num_classes_obj, num_classes_pred)
            self.gcn_update = _GraphConvolutionLayer_Update(num_classes_obj, num_classes_pred)
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        for t in range(self.update_step):
            ...
            source_rel_sub = self.gcn_collect(obj_scores[t], pred_scores[t], subj_pred_map, 0)
            source_rel_obj = self.gcn_collect(obj_scores[t], pred_scores[t], obj_pred_map, 1)
            source2obj_all = (source_rel_sub + source_rel_obj) / 2
            obj_scores.append(self.gcn_update(obj_scores[t], source2obj_all, 0))

            '''update predicate logits'''
            source_obj_sub = self.gcn_collect(pred_scores[t], obj_scores[t], subj_pred_map.t(), 2)
            source_obj_obj = self.gcn_collect(pred_scores[t], obj_scores[t], obj_pred_map.t(), 3)
            source2rel_all = (source_obj_sub + source_obj_obj) / 2
            pred_scores.append(self.gcn_update(pred_scores[t], source2rel_all, 1))

        return (x_obj, x_pred), obj_scores[-1], pred_scores[-1]
