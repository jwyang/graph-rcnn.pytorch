...
class Message_Passing_Unit_v1(nn.Module):
	...
	def forward(self, unary_term, pair_term):
		...
		gate = torch.sigmoid(self.w(gate)).sum(1)
		# print 'gate', gate
...
class IMP_BASE(nn.Module):
	def __init__(self, fea_size, dropout=False, gate_width=1, use_kernel_function=False):
		...
		self.vert_rnn = nn.GRUCell(fea_size, fea_size)
		self.edge_rnn = nn.GRUCell(fea_size, fea_size)
